...
int md5_file( char *path, unsigned char output[16] )
{
    ...
    EU_MEMSET( &ctx, 0, sizeof( md5_context ) );
...
int md5_self_test( int verbose )
{
    ...
    {
        if( verbose != 0 )
            CDebugLog( "  MD5 test #%d: ", i + 1 );

        md5( md5_test_buf[i], md5_test_buflen[i], md5sum );

        if( EU_MEMCMP ( md5sum, md5_test_sum[i], 16 ) != 0 )
        {
            if( verbose != 0 )
                CDebugLog( "failed\n" );
    ...
    {
        if( verbose != 0 )
            CDebugLog( "  HMAC-MD5 test #%d: ", i + 1 );

        if( i == 5 || i == 6 )
        {
            EU_MEMSET( buf, '\xAA', buflen = 80 );
        ...
        if( EU_MEMCMP ( md5sum, md5_hmac_test_sum[i], buflen ) != 0 )
        {
            if( verbose != 0 )
                CDebugLog( "failed\n" );
