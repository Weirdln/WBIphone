...
#pragma mark - Getting Username

+ (NSString *)usernameForService:(NSString *)service account:(NSString *)account {
    return [self usernameForService:service account:account error:nil];
}

+ (NSString *)usernameForService:(NSString *)service account:(NSString *)account error:(NSError **)error {
    NSData *data = [self usernameDataForService:service account:account error:error];
    if (data.length > 0)
    {
...
+ (NSData *)usernameDataForService:(NSString *)service account:(NSString *)account {
    return [self usernameDataForService:service account:account error:nil];
}

+ (NSData *)usernameDataForService:(NSString *)service account:(NSString *)account error:(NSError **)error {
    OSStatus  status = SSKeychainErrorBadArguments;
    if (!service || !account)
    {
        if (error)
...
#pragma mark - Deleting Username

+ (BOOL)deleteUsernameForService:(NSString *)service account:(NSString *)account {
	return [self deleteUsernameForService:service account:account error:nil];
}


+ (BOOL)deleteUsernameForService:(NSString *)service account:(NSString *)account error:(NSError **)error {
	OSStatus status = SSKeychainErrorBadArguments;
	if (service && account) {
		NSMutableDictionary *query = [self _queryForService:service account:account];
#if __has_feature(objc_arc)
...
#pragma mark - Setting Username

+ (BOOL)setUsername:(NSString *)Username forService:(NSString *)service account:(NSString *)account {
	return [self setUsername:Username forService:service account:account error:nil];
}


+ (BOOL)setUsername:(NSString *)Username forService:(NSString *)service account:(NSString *)account error:(NSError **)error {
    NSData *data = [Username dataUsingEncoding:NSUTF8StringEncoding];
    return [self setUsernameData:data forService:service account:account error:error];
}


+ (BOOL)setUsernameData:(NSData *)Username forService:(NSString *)service account:(NSString *)account {
    return [self setUsernameData:Username forService:service account:account error:nil];
}


+ (BOOL)setUsernameData:(NSData *)Username forService:(NSString *)service account:(NSString *)account error:(NSError **)error {
    OSStatus status = SSKeychainErrorBadArguments;
	if (Username && service && account) {
        [self deleteUsernameForService:service account:account];
        NSMutableDictionary *query = [self _queryForService:service account:account];
#if __has_feature(objc_arc)
		[query setObject:Username forKey:(__bridge id)kSecValueData];
#else
		[query setObject:Username forKey:(id)kSecValueData];
#endif
