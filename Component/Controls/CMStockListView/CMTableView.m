...
{
    return [self initWithFrame:frame pullDirection:CMTableViewDirectionNone];
}
...
- (id)initWithFrame:(CGRect)frame pullDirection:(CMTableViewDirection) cmDirection
{
    ...
    if (self) {
        ...
        if((cmDirection & (1 << EGOPullOrientationDown)) >> EGOPullOrientationDown)
        {
            // 头部，下拉时显示刷新
            _headView = [[EGORefreshTableHeaderView alloc] initWithScrollView:_tableView orientation:EGOPullOrientationDown];
            _headView.backgroundColor = [UIColor purpleColor];
            _headView.delegate = self;
        }
        
        if((cmDirection & (1 << EGOPullOrientationUp)) >> EGOPullOrientationUp)
        {
            // 底部，上拉时加载更多
            _footerView = [[EGORefreshTableHeaderView alloc] initWithScrollView:_tableView orientation:EGOPullOrientationUp];
            _footerView.backgroundColor = [UIColor cyanColor];
            _footerView.delegate = self;
        }
        
        if((cmDirection & (1 << EGOPullOrientationRight)) >> EGOPullOrientationRight)
        {
            // 左边，右拉时显示刷新
            _leftView = [[EGORefreshTableHeaderView alloc] initWithScrollView:_tableView orientation:EGOPullOrientationRight];
            _leftView.backgroundColor = [UIColor magentaColor];
            _leftView.delegate = self;
        }
        
        if((cmDirection & (1 << EGOPullOrientationLeft)) >> EGOPullOrientationLeft)
        {
            // 右边，左拉时加载更多
            _rightView = [[EGORefreshTableHeaderView alloc] initWithScrollView:_tableView orientation:EGOPullOrientationLeft];
            _rightView.backgroundColor = [UIColor brownColor];
            _rightView.delegate = self;
        }
        
        
        // tableView reload以后contentSize会发生变化，此时需要更新_headView
...
{
    if(pullDirection != CMTableViewDirectionNone)
    {
...
- (void)egoRefreshTableHeaderDidTriggerRefresh:(EGORefreshTableHeaderView*)view pullDirection:(EGOPullOrientation) cmDirection
{
    if(self.refreshDataDelegate != nil && [self.refreshDataDelegate respondsToSelector:@selector(refreshTableViewReloadTableViewDataSource:)])
    {
        if(cmDirection == EGOPullOrientationDown || cmDirection == EGOPullOrientationRight)
            [self.refreshDataDelegate refreshTableViewReloadTableViewDataSource:CMTableViewPullTypeReload];
        else if(cmDirection == EGOPullOrientationUp || cmDirection == EGOPullOrientationLeft)
            [self.refreshDataDelegate refreshTableViewReloadTableViewDataSource:CMTableViewPullTypeLoadMore];
    }
...
- (void)_refreshVisibleRows
{
    ...
    if([self.refreshDataDelegate respondsToSelector:@selector(refreshDataView:withRequestInfo:)])
    {
        ...
        [self.refreshDataDelegate refreshDataView:self withRequestInfo:[self _requestInfoForNeedRefreshRows:indexPaths]];
    }
}
...
- (void)_foreceReqDataWithRequestInfo:(NSArray *)requestInfos
{
    ...
    if([self.refreshDataDelegate respondsToSelector:@selector(refreshDataView:withRequestInfo:)])
    {
        [self.refreshDataDelegate refreshDataView:self withRequestInfo:requestInfos];
    }
}
